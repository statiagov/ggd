...
class EntitiesController < ApplicationController
  ...
  #----------------------------------------------------------------------------
  def entities
    instance_variable_get("@#{controller_name}") || klass.all
  end
  ...
  def get_list_of_records(options = {})
    ...
    unless advanced_search
      ...
      scope = scope.in_state(filter) if filter.present?
    end
  ...
  def set_view
    if params['view']
      ...
      session[:"#{controller}_#{action}_view"] = params['view']
    end
  end
end

...
module ApplicationHelper
  def tabs(tabs = nil)
    tabs ||= controller_path =~ /admin/ ? StatiaGov::Tabs.admin : StatiaGov::Tabs.main
    if tabs
      ...
      fail StatiaGov::MissingSettings, "Tab settings are missing, please run <b>rake ffcrm:setup</b> command."
    end
  end
  ...
  def entity_filter_radiobox(name, value, count)
    selected = (session["#{controller_name}_filter"].present? ? session["#{controller_name}_filter"].split(",").include?(value.to_s) : count.to_i > 0)
    url = url_for(action: :filter)
    onclick = %{
      var query = $('#query').val(),
          values = [];
    ...
    radio_button_tag("#{name}[]", value, selected, id: value, onclick: onclick)
  end
  ...
  def current_view_name
    ...
    session[:"#{controller}_#{action}_view"]
  end
  ...
  def template_for_current_view
    controller = params['controller']
    action = (params['action'] == 'show') ? 'show' : 'index' # create update redraw filter index actions all use index view
    template = StatiaGov::ViewFactory.template_for_current_view(controller: controller, action: action, name: current_view_name)
    template
  end
  ...
  def view_buttons
    controller = params['controller']
    action = (params['action'] == 'show') ? 'show' : 'index' # create update redraw filter index actions all use index view
    views = StatiaGov::ViewFactory.views_for(controller: controller, action: action)
    return nil unless views.size > 1
    lis = ''.html_safe
    content_tag :ul, class: 'format-buttons' do
      views.collect do |view|
        classes = if (view.template.nil?) # nil indicates default template.

...
module StatiaGov
  module Callback
    @@classes   = []  # Classes that inherit from StatiaGov::Callback::Base.
    ...
    # Adds a class inherited from from StatiaGov::Callback::Base.
    #--------------------------------------------------------------------------
    def self.add(klass)
      @@classes << klass
    end
    ...
    # This makes it possible to call hook() without StatiaGov::Callback prefix.
    ...
    module Helper
      def hook(method, caller, context = {}, &block)
        ...
        if is_view_hook
          hooks = StatiaGov::Callback.view_hook(method, caller, context)
          ...
          if hooks[:replace].empty?
            view_data << if block_given?
                           ...
                           # legacy view hooks
                           StatiaGov::Callback.hook(method, caller, context)
              end
          else
...
end # module StatiaGov

ActionView::Base.send(:include, StatiaGov::Callback::Helper)
ActionController::Base.send(:include, StatiaGov::Callback::Helper)

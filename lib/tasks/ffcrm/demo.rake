...
namespace :ffcrm do
  namespace :demo do
    ...
    task load: :environment do
      ...
      Dir.glob(StatiaGov.root.join('db', 'demo', '*.{yml,csv}')).each do |fixture_file|
        ActiveRecord::Fixtures.create_fixtures(StatiaGov.root.join('db/demo'), File.basename(fixture_file, '.*'))
      end

...
describe InspectionsController do
  def get_data_for_sidebar
    @states = Setting.inspection_states.dup
  end

  before(:each) do
    set_current_tab(:inspections)
  end
  ...
  describe "responding to GET index" do
    ...
    it "should expose all inspections as @inspections and render [index] template" do
      @inspections = [FactoryGirl.create(:inspection)]

      get :index
      expect(assigns[:inspections]).to eq(@inspections)
      expect(response).to render_template("inspections/index")
    end
